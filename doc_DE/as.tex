...
{\em CompactRISC} ist ein eingetragenes Warenzeichen von National
 Semiconductor (heute ein Teil von Texas Instruments).
...
{\em eZ80} und {\em Z80} sind eingetragene Warenzeichen von Zilog Inc.

{\em UNIX} ist ein eingetragenes Warenzeichen der Open Group.

{\em Linux} ist ein eingetragenes Warenzeichen von Linus Torvalds.
...
\item{AT\&T WE32100/32200 (''Bellmac32'')}
...
\item{Zilog Z80 (einschlie"slich undokumentierter Befehle), Z180, Z280, Z380, eZ80}
...
\item{Texas Instruments TMS340xx}
...
--- mit einer Breite von 32, 64, oder 128 Bit f"ur Integerwerte, 80/64 Bit f"ur
...
\ttindex{SHARED}
\begin{itemize}
\item{\tty{l}: Assembler-Listing auf Konsole ausgeben.  Falls mehrere
      Passes ausgef"uhrt werden m"ussen, landen im Gegensatz zur
      n"achsten Option die Listings aller Durchg"ange auf der Ausgabe!}
\item{\tty{L}: Assembler-Listing auf Datei schreiben.  Die Listdatei erh"alt
...
\item{\tty{listline-prefix}: Mit dieser Option k"onnen die Daten
      bestimmt werden, die im Listing jeder Zeile vorangestellt
      werden.  Der Aufbau des daf"ur verwendeten Formatstrings ist in
      \ref{SectListing} beschrieben.}
\item{{\tt list-unknown-values} bzw. {\tt no-list-unknown-values}: Schaltet
      die Anzeige von im ersten Durchlauf unbekannten Werten als Fragezeichen
      an bzw. aus.}
\item{\label{radix}\tty{RADIX}: Stellt das Default-Zahlensystem auf
      einen Wert zwischen 2 und 36, anstelle von 10.  Dieser Wert kann
      durch die gleichnamige Anweisung im Programm "ubersteuert werden.}
\item{\tty{listing}: Gibt vor, ob im Listing des assemblierten Codes alles
      ausgegeben werden soll, oder durch bedingte Assemblierung ausgelassenene
      Teile nicht.  Die erlaubten Werte sind die gleichen wie in \ref{SectListingCmd}
      beschrieben.}
...
\item{\tty{relaxed}: Mit diesem Schalter aktiviert man den RELAXED-Modus
      ...
      werden mu"s.}
\item{\tty{intsyntax}: Mit diesem Schalter kann man vom Beginn des Programms
      an die Liste der erlaubten Schreibweisen f"ur Ganzzahl-Konstanten
      erweitern oder reduzieren, analog zur gleichnamigen Pseudoanweisung
      (siehe Abschnitt \ref{SectINTSYNTAX} f"ur eine Liste erlaubter
      Argumente).}
...
\item{\tty{extracomments}: Mit diesem Schalter erlaubt man vom Beginn
      an die Verwendung alternativer Methoden, um Kommentare im Programm
      unterzubringen (siehe Abschnitt \ref{SectEXTRACOMMENTS}).}
...
\item{{\tt maxsympass [n]}: Definiert, bis zu welchen Pass unaufgel"oste
      Vorw"artsreferenzen erlaubt sind.  Eine Umstellung auf einen
      Wert gr"o"ser als den Vorgabewert (1) kann in Sonderf"allen, wie
      bei verketteten Vorw"artsreferenzen erforderlich sein.}
\item{{\tt Werror}: Weist den Assembler an, Warnungen als Fehler zu
      behandeln.}
\item{{\tt wrelative} bzw. {\tt wno-relative}: Weist den Assembler an,
      Warnungen auszugeben, falls ein relativer anstelle eines absoluten
      Sprungs m"oglich ist (nur f"ur Z80-Target).}
\item{{\tt wimplicit-sign-extension} bzw. {\tt wno-implicit-sign-extension}: Weist den
      Assembler an, Warnungen "uber implizite Vorzeichen-Erweiterungen
      auszugeben (nur 68K, MOVEQ).}
...
\par
Alles was hinter einem Semikolon steht, wird als Kommentar betrachtet
und bei der Assemblierung nicht weiter ausgewertet.  Je nachdem
wo es in der Zeile steht, kann diese dann auch nur ein Label und/oder
einen Befehl enthalten, oder g"anzlich nur aus einem Kommentar bestehen.
F"ur einzelne Targets existieren weitere M"oglichkeiten, Kommentare als
solche zu kennzeichnen:
\begin{itemize}
\item{Bei Padauk-Controllern k"onnen Kommentare anstelle eines
      Semikolons auch mit einem doppelten Schr"agstrich gekennzeichnet
      werden, wie man es von C++ kennt.}
\item{Schaltet man die \tty{EXTRACOMMENTS}-Option (siehe \ref{SectEXTRACOMMENTS})
      bei 68xx-Targets ein, so wird der Rest der Zeile als Kommentar
      betrachtet, falls Label oder Mnemonic mit einem Stern beginnen.}
\item{Ebenfalls bei eingeschalteter \tty{EXTRACOMMENTS}-Option wird
      die Argumentliste durch ein Leerzeichen abgeschlossen und alles
      danach als Kommentar betrachtet.}
\end{itemize}
\par
...
\begin{enumerate}
\item{Erg"anzte Wiedergabe des assemblierten Quellcodes;}
\item{Symbolliste;}
\item{Makroliste;}
\item{Funktionsliste;}
\item{Belegungsliste;}
...
\par
Der erzeugte Code selber steht dahinter im Feld \tty{Code}, ebenfalls
im durch den List-Radix gegebenen Zahlensystem.  Je nach Prozessortyp und
aktuellem Segment bzw. Adre"sraum k"onnen die Werte dabei mit unterschiedlicher
L"ange formatiert sein.  Das k"onnen Bytes mit 8 Bits sein, aber auch Worte
mit 16 oder 32 Bits, wenn der gerade genutzte Adre"sraum nicht Byte-adressierbar
ist, oder die Instruktionsworte des Prozessors l"anger als 8 Bit sind.  Dies
ist z.B. beim 68000 oder der PDP-11 der Fall: Deren Adre"sraum ist Byte-weise
organisiert, Instruktionsworte sind aber 16 Bit lang.  Bei einigen wenigen
Targets sind auch L"angen m"oglich, die nicht ein mehrfaches von 8 Bit betragen,
zum Beispiel 12 Bit bei der PDP-8.
\par
Bei Vorw"artsreferenzen gibt es im ersten Durchlauf die Situation, dass der
endg"ultige Code gar nicht angegeben werden kann. \asname{} macht dann eine
Annahme f"ur den Wert, der die weitere Assemblierung nicht st"ort.  Bei einer
(zunehmenden) Zahl von Targets werden diese Felder im Code auch explizit
mit Fragezeichen angezeigt.  Die folgenden Targets unterst"utzen dies aktuell:
\begin{itemize}
\item{68xxx/Coldfire}
\item{DSP56000/56300}
\item{PowerPC}
\item{65xx/65xxx}
\item{6800/6805/68HC08/6809/68HC11/68HC12}
\item{8080/8085}
\item{(e)Z80/Z180/Z280/Z380}
\item{8048}
\item{8051/80251}
\item{8096/80196/80296}
\item{8086/V20...V55}
\item{AVR}
\item{TMS370xxx}
\item{uCOM-43}
\end{itemize}
Falls die Fragezeichen beim Weiterverarbeiten des Listings st"oren, k"ann
diese Funktion "uber den Kommandozeilen-Schalter \tty{-no-list-unknown-values}
abgeschaltet werden.
\par
...
\par
Die Struktur der Daten vor der Quellcode-Zeile wird intern
durch einen Formatstring gesteuert, der "uber die Kommandozeilen-Option
{\tt -listline-prefix} ge"andert werden kann.  Dieser darf folgende
Platzhalter enthalten:
\begin{itemize}
\item{{\tt \%[c]i}: Die aktuelle Include-Verschachtelungstiefe, mit optionaler
      Feldbreite.  Ohne Feldbreite wird dies auf gar nichts oder eine
      passende Menge Leerzeichen expandiert, falls die Tiefe Null ist.}
\item{{\tt \%[c]n}: Die aktuelle Zeilennummer, mit einer optionalen Feldbreite,
      in der die Zeilennummer rechtsb"undig ausgegeben wird.  Der Default
      f"ur die Feldbreite ist f"unf Zeichen.}
\item{{\tt \%[c]a}: Analog die aktuelle Speicheradresse, wobei der Default
      der Feldbreite der oben erw"ahnte, Target-abh"angige Wert ist.}
\end{itemize}
In allen drei F"allen bedeutet eine f"uhrende Null bei einer Feldbreite, da"s
links mit Nullen anstelle von Leerzeichen aufgef"ullt werden soll.
Der Default f"ur diesen Formatstring ist \verb!%i%n/%a!.  Um eine Ausgabe
zu erhalten, wie sie Versionen von \asname{} vor 1.42 Build 249 erzeugten,
kann als Formatstring \verb!%1i%5n/%8a! verwendet werden.
\par
...
\tty{INTWIDTH}         & Bitbreite interne Integer-Arithmetik \\
\tty{HAS64}            & interne Integer-Arithmetik verwendet mindestens 64 Bit \\
\tty{FLOATMAX}         & gr"o"ste darstellbare Gleitkommazahl \\
...
\tty{ARCHITECTURE}     & Zielplattform, f"ur die \asname{} "ubersetzt wurde, \\
                       & in der Form Prozessor-Hersteller-Betriebssystem \\
\tty{DATE}             & Datum und \\
...
\par
Auf den meisten Plattformen ist der Name {\tt INF} als Sonderwert
f"ur Unendlich im Gleitkomma-Format reserviert und kann nicht
f"ur eigene Symbole verwendet werden.
\par
...
Ob ein Symbol definiert ist, kann mit den Funktionen \tty{DEFINED} bzw.
\tty{SYMEXIST} gepr"uft werden.  Der Unterschied: \tty{SYMEXIST} liefert
bei einer Vorw"artsdeklaration im ersten Durchlauf ein 'false' und bei
weiteren Durchl"aufen ein 'true'.  Im Gegensatz dazu liefert \tty{DEFINED} nur
dann ein 'true', wenn das Symbol im bis zu diesem Punkt assemblierten Code
definiert wurde.  Bei Vorw"artsdeklarationen bleibt es also bei einem 'false',
unabh"angig vom Durchlauf.

\label{sect_symused}
Mittels der Funktion \tty{SYMUSED} kann abgefragt werden, ob ein Symbol
bisher schon einmal verwendet wurde.  Dies schlie"st auch Bez"uge auf
ein Symbol ein, die {\em vor} seiner Definition erfolgen, falls
eine der folgenden Bedingungen erf"ullt ist:
\begin{itemize}
\item{Sowohl die Vorw"artsreferenz als auch die Definition erfolgen im
      globalen Symbol-Namensraum, d.h. au"serhalb von Sektionen oder
      expandierten Makro-Rümpfen.}
\item{Die Vorw"artsreferenz erfolgt in einer Sektion, aber au"serhalb
      eines expandierten Makro-Rumpfes, und die Definition erfolgt ebenfalls
      au"serhalb eines expandierten Makro-Rumpfes, in der selben Sektion, oder
      einer Eltern-Sektion dieser Sektion, oder global.}
\item{Die Vorw"artsreferenz erfolgt innerhalb eines expandierten Makro-Rumpfes,
      und die Definition im selben Makro-Rumpf.}
\end{itemize}
Falls ein Symbol auf Benutzung gepr"uft werden soll und es Vorw"artsreferenzen
gibt, die keine dieser drei Situationen erf"ullt, bleibt die M"oglichkeit,
ein Symbol vor der ersten Vorw"artsreferenz mittels des \tty{FORWARD}-Befehls
anzuk"undigen.  Es wird dann quasi ein 'Leereintrag' in der Symboltabelle
angelegt, an dem Vorw"artsreferenzen explizit vermerkt werden k"onnen.

\tty{DEFINED} akzeptiert nicht nur einen einfachen Symbolnamen, sondern auch
einen kompletten Formelausdruck.  Der R"uckgabewert ist in so einem Fall
nur dann 'true', falls der Ausdruck keine Symbole enth"alt, die nach der im
vorigen Absatz gegebenen Erkl"arung undefiniert sind.
\begin{table}[htb]
\begin{center}
\begin{tabular}{|l|c|}
\hline
...
Besonders bei Programmen mit vielen aufeinander folgenden Schleifen oder
...
\subsection{Integer-Konstanten}
\label{SectIntConsts}

Integer-Konstanten bezeichnen ganze Zahlen.  Sie werden als eine
...
vorgegeben wird, wird die Basis folgenderma"sen bestimmt:
\begin{itemize}
\item{Falls ein {\tt RADIX}-Befehl gegeben wurde, benutze die dadurch
      definierte Basis, ansonsten:}
\item{Falls ein {\tt -radix} Kommandozeilenschalter gegeben wurde,
      benutze die dadurch definierte Basis, ansonsten:}
\item{Verwende das Dezimalsystem (Basis 10).}
\end{itemize}
Sowohl mit dem{\tt RADIX}-Befehl als auch mit dem {\tt -radix}-Kommandozeilenschalter
lassen sich auch ,,ungew"ohnliche'' Zahlensysteme, d.h. andere als 2, 8,
10 oder 16 einstellen.
...
Die Verwendung von Buchstaben in Integer-Konstanten bringt allerdings auch
...
9 beginnen, was bedeutet, da"s eine Integer-Konstante, die nicht durch ein
...
zwischen A und F, so hilft das hintan gestellte H "uberhaupt nichts, es
mu"s noch eine Null davor (statt F0H also 0F0H).  Die Motorola-oder
C-Syntax, die beide das Zahlensystem am Anfang einer Integer-Konstante
...
Integer-Konstante zu verstehen und macht erst dann einen Versuch mit
...
auch in als ASCII definierten Integer-Konstanten zul"assig, z.B. so:
\begin{verbatim}
   move.b   #'\n',d0
\end{verbatim}
...
erfolgt immer mit der h"ochsten auf dem Host-System verf"ugbaren Wortbreite.
Bei Ganzzahlen sind dies 32 oder 64 Bit, und bei Gleitkommazahlen
ca. +/-$1.8*10^{308}$ (IEEE Double Precision) oder ca. +/-$1.1*10^{4932}$
(IEEE Extended Precision).  Eine eventuelle Pr"ufung auf
Wertebereichs"uberschreitungen findet erst am Endergebnis statt.
...
\hline
\end{tabular}\end{center}
\caption{in \asname{} definierte Operatoren\label{TabOps}}
\end{table*}
...
ansonsten FALSE.
\par
Grunds"atzlich ist bei Operatoren mit zwei Argumenten nicht definiert, in
welcher Reihenfolge die Operanden bestimmt werden.  Die einzige Ausnahme
von dieser Regel sind das logische AND und OR: Falls das Ergebnis bereits
durch den linken Operanden eindeutig bestimmt ist, wird der rechte
Operand nicht mehr ausgewertet,
\par
...
dar"uber liegenden Bits aber unver"andert.  Die Zahl der zu spiegelnden
...
Zus"atzlich zu den Operatoren definiert \asname{} noch eine Reihe
...
EXPRTYPE    & liefert Typ des       & Integer,     & 0 \\
            ...
            &                       & String       & 2 \\
FSIZE       & liefert Dateigr"o"se  & String       & Integer \\
\hline
...
\tty{FSIZE} liefert die Gr"o"se einer Datei in Bytes.  Die
Datei wird sowohl im aktuellen Verzeichnis gesucht, als auch
relativ zum Verzeichnis der gerade assemblierten Quelldatei.
Im Gegensatz zu \tty{INCLUDE} und \tty{BINCLUDE} wird jedoch
nicht im Include-Pfad gesucht!
...
\item{Entfernen Sie diese verfluchte Vorw"artsreferenz und
...
{\em G"ultigkeit: PALM, 8008/8080/8085/8086, XA, Z80, Z8000, 320C2x/5x, TLCS-42/47, AVR,
     F8, IMP-16}
...
{\em G"ultigkeit: 680x0, PowerPC, PALM, M*Core, XGate, H8,\\
     SH7x00, H16, M16(C), PDP-11, WD16, VAX, 4004, MCS-48/51,\\
     8086, i960, XA, AVR, 29xxx,  80C16x, Z8, Z80, Z8000,\\
     KCPSM(3), LatticeMico8, TLCS-9000, ST9, MSP430(X), V60,\\
     SC/MP, NS32xxx, WE32xxx, XCore, KENBAK\\
...
Auf PDP-11 und TMS340xx darf \tty{REG} zus"atzlich ohne Namen im Label-Feld benutzt
werden.  Es wird dann als einziges Argument entweder \tty{ON} oder \tty{OFF}
erwartet, und damit werden die eingebauten Register-Aliase:
\begin{itemize}
\item{PDP-11: \tty{Rn} = \tty{\%n}, \tty{SP} = \tty{R6}, \tty{PC} = \tty{R7}}
\item{TMS340xx: \tty{SADDR} = \tty{B0}, \tty{SPTCH} = \tty{B1},
                \tty{DADDR} = \tty{B2}, \tty{DPTCH} = \tty{B3},
                \tty{OFFSET} = \tty{B4}, \tty{WSTART} = \tty{B5},
                \tty{WEND} = \tty{B6}, \tty{DYDX} = \tty{B7},
                \tty{COLOR0} = \tty{B8}, \tty{COLOR1} = \tty{B9},
                \tty{COUNT} = \tty{B10}, \tty{INC1} = \tty{B11},
                \tty{INC2} = \tty{B12}, \tty{PATTRN1} = \tty{B13}}
\end{itemize}
ein- und ausgeschaltet.  Sie
...
\hline
\multicolumn{11}{|l|}{$^{2}$ Da Z180 und Z280 weiterhin logisch nur 64K ansprechen k"onnen, ist der} \\
\multicolumn{11}{|l|}{ganze Adre"sraum nur mittels \tty{PHASE}-Anweisungen oder passenden \tty{ASSUME}-} \\
\multicolumn{11}{|l|}{Werten f"ur die MMU-Register erreichbar.} \\
\hline
...
\hline
\multicolumn{11}{|l|}{$^{11}$ Nur die ersten 128 KByte sind f"ur ausf"uhrbaren Code nutzbar.} \\
\hline
...
%%-----------
\begin{cpulist}
   PPC403 $\rightarrow$ PPC403GC $\rightarrow$ MPC505 $\rightarrow$ MPC601 $\rightarrow$ MPC821 $\rightarrow$ RS6000
\end{cpulist}
...
%%-----------
\begin{cpulist}
   6809/6809UNDOC/6309 und 6805/68HC08/68HCS08
\end{cpulist}
...
deutlich mehr Befehle besitzt (siehe \cite{Kaku}).  6809UNDOC schaltet
die undokumentierte {\tt HCF}-Instruktion frei.
%%-----------
\begin{cpulist}
...
%%-----------
\begin{cpulist}
   HD6413308 $\longrightarrow$ HD6413309
\end{cpulist}
...
%%-----------
\begin{cpulist}
   HD6475328 $\longrightarrow$ HD6475348 $\longrightarrow$
...
%%-----------
\begin{cpulist}
   SH7000 $\longrightarrow$ SH7600 $\longrightarrow$ SH7700
\end{cpulist}
...
%%-----------
\begin{cpulist}
   6502 $\rightarrow$ 65(S)C02 $\rightarrow$ 65CE02 / W65C02S\\
...
\> VAX-8650, VAX-8800
\end{cpulist}
Alle Implementierungen der VAX-Architektur unterst"utzen den
gleichen Kern-Befehlssatz.  Einige Erweiterungen, wie z.B.
Befehle zur Verarbeitung von Strings, gepackten Dezimalzahlen
oder bestimmten Gleitkommaformaten, sind nicht immer in
Hardware implementiert.
%%-----------
\begin{cpulist}
   WE32100 $\rightarrow$ WE32200
\end{cpulist}
Die dritte Generation des WE32xxx-Prozessors bietet sechzehn
zus"atzliche CPU-Register, und mit diesen zusammen neue
Adressierungsmodi wie Indiziert oder Auto-Inkrement und -Dekrement.
Des weiteren wurden neue Maschinenbefehle hinzu gef"ugt.
%%-----------
\begin{cpulist}
   CP-3F, LP8000, M380
\end{cpulist}
...
\end{cpulist}
Intel hat 1975 die Mnemonics des 8008 umdefiniert, die zweite Variante spiegelt
...
%%-----------
\begin{cpulist}
   AT90S1200, AT90S2313, AT90S2323, AT90S233, AT90S2343,\\
...
%%-----------
\begin{cpulist}
   80C166 $\longrightarrow$ 80C167,80C165,80C163
\end{cpulist}
...
Die Zilog eZ80 Varianten erweitern die Z80 Architektur um
einem 16 MByte gro"sen Adre"sraum, 24 Bit breite Register
sowie eine begrenzte Zahl neuer Befehle.  Einige Varianten
enthalten ein I-Register, das nur 8 Bit breit ist.  Dem
eZ80190 fehlen zus"atzlich einige String-I/O Befehle.  Ansonsten
liegen die Unterschiede nur in der Menge des eingebauten
Speichers sowie der integrierten Peripherie.
%%-----------
\begin{cpulist}
...
%%-----------
\begin{cpulist}
4240P, 4250N, 4260P, 4270N \\
42C00Y, 42C40P, 42C50N, 42C60P, 42C70N\\
\end{cpulist}
Die CMOS-Varianten verf"ugen "uber zwei zus"atzliche Befehle.  Ansonsten
unterscheiden die Varianten sich in der Anzahl der I/O-Ports und der
Gr"o"se des Prorammspeichers.
%%-----------
\begin{cpulist}
   97C241
\end{cpulist}
...
%%-----------
\begin{cpulist}
   34010 $\rightarrow$ 34020
\end{cpulist}
Der TMS34020 unterst"utzt eine vollen 4 GBit-Adres"raum, zus"atzliche
Maschinenbefehle sowie eine Koprozessor-Schnittstelle.
%%-----------
\begin{cpulist}
...
%%-----------
\begin{cpulist}
   370C010, 370C020, 370C030, 370C040 und 370C050
\end{cpulist}
...
%%-----------
\begin{cpulist}
   CR16A $\rightarrow$ CR16B $\rightarrow$ CR16C
\end{cpulist}
CR16B erweitert den Adre"sraum von 256 KByte auf 2 MByte und stellt
eine Reihe neuer Befehle zur Verf"ugung.  CR16C erweitert ihn nochmals
auf 16 MByte, ist aber nur noch Quellcode- und nicht mehr bin"ar
aufw"artskompatibel. 
%%-----------
\begin{cpulist}
   ACE1101, ACE1202
...
\end{cpulist}
Die drei Gruppen vom Controllers geh"oren jeweils zur
$\mu$COM-45-, $\mu$COM-44- und $\mu$COM-43-Familie. Die
ersten beiden Familien implementieren eine Untermenge
des $\mu$COM-43-Befehlssatzes.  Ansonsten unterscheiden
die Varianten sich in der Menge des integrierten ROM-
und RAM-Speichers.  iso650 ist eine FPGA-Implementierung
des uPD650 und unterscheidet sich vom 'Original' durch
eine NAND-Anweisung sowie einen von 2000 auf 4096 Byte
vergr"o"serten Adre"sraum.
%%-----------
\begin{cpulist}
   7566 $\leftrightarrow$ 7508
\end{cpulist}
...
%%-----------
\begin{cpulist}
   1802 $\rightarrow$ 1804, 1805, 1806 $\rightarrow$ 1804A, 1805A
1806A
\end{cpulist}
...
\par
Eine wichtige "Anderung: Das Default-Target ist nicht mehr der 68008.  Falls
...
{\em
\begin{itemize}
\item{G"ultigkeit SUPMODE: 680x0, NS32xxx, CR16C, PDP-11, i960,
      TLCS-900, SH7000, i960, 29K, Z280, XA, PowerPC, M*CORE,
      V60 und TMS9900}
\item{G"ultigkeit FPU: 680x0, NS32xxx, 80x86, WE32xxx}
...
\subsection{ACCMODE bzw. EXECMODE}
\ttindex{ACCMODE}
\ttindex{EXECMODE}

{\em
\begin{itemize}
\item{G"ultigkeit: VAX (ACCMODE),\\
      WE32xxx (EXECMODE)}
\end{itemize}
}

VAX und WE32xxx kennen nicht nur einen Supervisor- un User-Mode, sondern vier derartige
Privilegstufen.  Mit absteigenden Rechten heissen diese Zugriffs-Modi Kernel,
Executive, Supervisor und User.  Mit dem {\tt ACCMODE} bzw. {\tt EXECMODE}-Befehl teilt man dem
Assembler mit, in welchen Modus der folgende Code ausgef"uhrt wird.  Je nach
Modus sind nicht alle Maschinenbefehle erlaubt.  Als Argument sind entweder
die genannten Namen der vier Modi zul"assig, oder eine Zahl von Null
(Kernel-Modus) bis Drei (User-Modus).  Im Default wird der User-Modus angenommen,
und die aktuelle Einstellung (als Zahlenwert) l"asst sich aus dem Symbol gleichen
Namens lesen.
...
{\em G"ultigkeit: TMS34020}

Mit diesem Befehl wird festgelegt, welche Id (Adresse) in Koprozessor-
Befehle eingesetzt werden soll, wenn keine explizite Id angegeben wurde.
Der Default f"ur diesen Wert wiederum ist Null.  Erlaubte Werte sind von
0 bis 7.
...
\subsection{EXTRACOMMENTS}
\ttindex{EXTRACOMMENTS}\label{SectEXTRACOMMENTS}

{\em G"ultigkeit: 68xx}

Motorolas Original-Assembler haben das Feature, dass wenn Label oder
Mnemonic mit einem Stern beginnen, der Rest der Zeile als Kommentar
betrachtet wird.  \asname{} unterst"utzt dies normalerweise nicht.  Man
kann dies aber mit einem
\begin{verbatim}
  extracomments on
\end{verbatim}
einschalten, um die Portierung von Code zu vereinfachen, der für andere
Assembler geschrieben wurde.

Des weiteren werden sogenannte 'End-of-Line' Kommentare zugelassen:
Die Argumentliste endet bei ersten (nicht-gequoteten) Leerzeichen, alles
danach wird als Kommentar interpretiert.

Es ist m"oglich, diesen Modus "uber eine gleichnamige Kommandozeilen-Option
von Beginn einzuschalten.  Der aktuelle Wert kann aus einer Variable gleichen
Namens ausgelesen werden.

%%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\subsection{PLAINBASE}
...
DQ, DT} und \tty{DO} mit Big- oder Little-Endian-Orientierung arbeiten sollen.
...
\subsection{WARNRELATIVE}

{\em G"ultigkeit: Zx80}

Mit diesem Schalter teilt man dem Assembler mit, ob eine Warnung erfolgen soll,
falls ein relativer anstelle eines absoluten Sprungs m"oglich gewesen w"are.
Der Default ist OFF bzw. das, was auf der Kommandozeile durch die Argumente
{\tt -wrelative} bzw. {\tt -wno-relative} vorgegeben wurde.
...
gesteuert.  "Ahnlich wie beim 68HC11K4 nimmt \asname{} automatische Umrechnungen
von physischen auf logische Adressen vor, und zwar sowohl f"ur absolute Adressen
als auch Quelle und Ziel von relativen Spr"ungen.  Es ist ebenfalls m"oglich, auf
die Mapping-Tabellen "uber die Funktionen {\tt phys2cpu()} und {\tt cpu2phys()}
zuzugreifen.
...
Beim Z280 kann man "uber das I-Bit im Trap Control Register festlegen, ob
I/O-Zugriffe im User-Modus erlaubt sein sollen oder nicht.  Teilt man \asname{}
"uber ein
\begin{verbatim}
   assume i:1
\end{verbatim}
mit, dass sie verboten sind, dann wird eine entsprechende Warnung ausgegeben,
falls I/O-Befehle im User-Modus verwendet werden.

Des weiteren enth"alt der Z280 eine MMU, um den logischen 64K-Adre"sraum der
CPU auf die physikalisch adressierbaren 16 MByte abzubilden.  Ihre Funktion
wird zum einen "uber das MMU Master Control Register ({\tt MMUMCR}) gesteuert,
zum anderen "uber 32 Page Descriptor Register (({\tt UPD0...UPD15},
{\tt SPD0...SPD15}).  Die aktuellen Werte aller Register k"onnen \asname{} via
{\tt ASSUME}-Anweisungen mitgeteilt werden.

F"ur die Berechnung des aktuellen Mappings werden die folgenden Bits
des {\tt MMUMCR} beachtet:
\begin{itemize}
\item{Bit 15 (UTE): MMU im User-Modus einschalten}
\item{Bit 14 (UPD): Programm/Data-Separation im User-Modus einschalten}
\item{Bit 11 (STE): MMU im System-Modus einschalten}
\item{Bit 10 (SPD): Programm/Data-Separation im System-Modus einschalten}
\end{itemize}

Die folgenden Bits von {\tt UPDn/SPDn} werden beachtet:
\begin{itemize}
\item{Bit 3 (V): Seite g"ultig/aktiv}
\item{Bit 15...4 bzw. 15...5: Seitenadresse, also Bits 23...12
      bzw. 23...13 der Startadresse im physischen Adressraum.}
\end{itemize}

Abh"angig von den Registerwerten kann die Adress"ubersetzung im System-
und/oder User-Modus aktiv sein (STE und/oder UTE gesetzt), oder "uberhaupt
nicht (sowohl STE als auch UTE nicht gesetzt).  Die aktuelle Einstellung
von {\tt SUPMODE} hat deshalb auch einen Einflu"s auf das aktuelle Mapping.
Falls es eingeschaltet ist, kann es f"ur Code- und Datenzugriffe gleich
oder unterschiedlich sein (UPD/SPD nicht gesetzt oder gesetzt).
...
\begin{itemize}
\item{Im Z80 Modus sind die Register BC, DE, HL, IX, IY und SP 16 bit breit,
      und nur die 64K-Speicherseite, die durch das MBASE-Register definiert
      ist, kann adressiert werden.  Bei Befehlen, die eine absolute Adresse oder
      einen nicht-8-Bit Wert als Argument beinhalten, werden zwei Bytes
      geladen.}
\item{Im ADL Modus sind die Register BC, DE, HL, IX, IY und SP 24 Bit breit,
      und der komplette 16 MByte Adressraum kann angesprochen werden.  Bei Befehlen,
      die eine absolute Adresse oder einen nicht-8-Bit Wert als Argument beinhalten,
      werden drei Bytes geladen.}
\end{itemize}
Da die Bin"arkodierung und Adre"sbereichspr"ufung von diesem Modus abh"angt,
mu"s der Assembler wissen, welcher Modus gerade verwendet wird.  Indem man
{\tt ADL} mittels {\tt ASSUME} entweder auf 0 oder 1 setzt, teilt man dem
Assembler den Default-Modus mit, d.h. den Modus ohne Verwendung expliziter
Suffixe an einem Befehl.  Des weiteren kann dem Assembler der aktuelle
Wert von MBASE (0 bis 0ff hex) mitgeteilt werden.  Die Default-Annahme
f"ur beide Werte ist 0, "ahnlich den Werten nach einem Hardware-Reset.
...
128 Worte.  Der Default ist ersteres, mit einem {\tt ASSUME BPS:1} schaltet man
...
Der CR16C kann in einer alternativen Betriebsart laufen, in der die Register
\tty{R12...SP} auf 16 Bit beschr"ankt sind, um eine bessere architektonische
Aufw"artskompatibilit"at zu seinen Vorg"angern zu erreichen.  Die Betriebsart
wird durch das \tty{SR}-Bit im \tty{CFG}-Register aktiviert und hat die folgenden
weiteren Effekte auf das Programmiermodell:
\begin{itemize}
\item{Weitere 16-Bit-Registerpaare: \tty{(R13\_L,R12\_L), (RA\_L,R13\_L),
      (SP\_L,RA\_L), (SP\_H,SP\_L)}.}
\item{Indexregister-Adressierung (\tty{[Rn]abs20, [Rn]disp(rp)}) ist
      nicht erlaubt.}
\item{Einige Befehle erlauben kürzere Kodierung f"ur den Adressierungsmodus
      \tty{disp(reg)}, falls das Displacement Null oder kleiner als 0x4000
      ist.}
\end{itemize}
Der Wert dieses Bits zur Laufzeit kann "uber die Anweisung
\begin{verbatim}
   assume sr:n
\end{verbatim}
mitgeteilt werden, wobei \tty{n} entweder Null oder Eins ist.  Null ist der
Default, d.h. alle Register haben ihre volle Breite und Indexadressierung
ist erlaubt.
...
\subsection{BLKB, BLKW, BLKL, BLKD}
...
\subsection{DN,DB,DW,DD,DQ,DT \& DO}
...
\item{\tty{DW}: 16-Bit-Integer oder Gleitkomma mit halber Genauigkeit}
\item{\tty{DD}: 32-Bit-Integer oder Gleitkomma mit einfacher Genauigkeit}
\item{\tty{DQ}: 64-Bit-Integer oder Gleitkomma mit doppelter Genauigkeit}
\item{\tty{DT}: Gepackter BCD Integer oder Gleitkomma mit erweiterter Genauigkeit (80 Bit)}
\item{\tty{DO}: Gleitkomma mit vierfacher Genauigkeit (128 Bit)}
\end{itemize}
...
zu verwenden.
\par
\tty{DB} und \tty{DW} auf 65xx- und 68xx-Plattformen verstehen neben
dem \tty{DUP}-Operator zus"atzlich die 'Motorola-Variante' davon,
n"amlich eine in eckigen Klammern davor geschriebene Zahl von
Wiederholungen.
\par
...
\item{\tty{BYTE} bzw. \tty{WORD} funktionieren auf der PDP-11, VAX und
      WD16 wie \tty{DB} bzw. \tty{DW}, akzeptieren aber nur Ganzzahl-Argumente.}
\item{\tty{LWORD} bzw. \tty{QUAD} funktionieren auf der VAX wie \tty{DD}
      bzw. \tty{DQ}, akzeptieren aber nur Ganzzahl-Argumente.}
\ttindex{HALF}
\item{\tty{BYTE} funktioniert auf WE32xxx wie \tty{DB}, \tty{HALF}
      wie \tty{DW}, und \tty{WORD} wie \tty{DD}, akzeptiert aber nur
      Ganzzahl-Argumente.}
\item{\tty{DOUBLE} funktionieren auf WE32xxx wie \tty{DQ}, akzeptiert
      aber nur Gleitkomma-Argumente)}
\ttindex{BYTE}\ttindex{STRING}
\ttindex{WORD}\ttindex{LONG}\ttindex{INT}
\ttindex{FLOAT}\ttindex{DOUBLE}
\item{Auf dem TI TMS340xx existieren:
      \begin{itemize}
      \item{\tty{BYTE} (wie \tty{DB}, nur Ganzzahl-Argumente)}
      \item{\tty{STRING} (wie \tty{DB}, nur Zeichenketten-Argumente)}
      \item{\tty{WORD} (wie \tty{DW}, nur Ganzzahl-Argumente)}
      \item{\tty{LONG} oder \tty{INT} (wie \tty{DD}, nur Ganzzahl-Argumente)}
      \item{\tty{FLOAT} (wie \tty{DD}, nur Gleitkomma-Argumente)}
      \item{\tty{DOUBLE} (wie \tty{DQ}, nur Gleitkomma-Argumente)}
      \end{itemize}}
\ttindex{LWORD}\ttindex{QUAD}
\item{Auf dem Renesas RX existieren:
      ...
      \item{\tty{DOUBLE} (wie \tty{DQ}, nur Gleitkomma-Argumente)}
      \end{itemize}}
\item{Auf CR16 existieren:
      \begin{itemize}
      \item{\tty{BYTE} oder \tty{DC8} (wie \tty{DB}, Strings oder Ganzzahl-Argumente)}
      \item{\tty{WORD} oder \tty{DC16} (wie \tty{DW}, nur Ganzzahl-Argumente)}
      \item{\tty{LONG} oder \tty{DC32} (wie \tty{DD}, nur Ganzzahl-Argumente)}
      \item{\tty{DC64} (wie \tty{DQ}, nur Ganzzahl-Argumente)}
      \item{\tty{FLOAT} oder \tty{DF32} (wie \tty{DD}, nur Gleitkomma-Argumente)}
      \item{\tty{DOUBLE} oder \tty{DF64} (wie \tty{DQ}, nur Gleitkomma-Argumente)}
      \end{itemize}}
\end{itemize}
...
akzeptiert aber neben String- und Integer-Argumenten auch Gleitkommawerte
(und legt sie prozessorspezifischen 32-Bit-Format ab). \tty{DUP} gibt es {\em
nicht}!

Bei der Ablage von Gleitkommazahlen ist zu beachten, da"s diese keine h"ohere
Genauigkeit und Wertebereich haben k"onnen als das auf dem Host-System verwendete
Format.  Handelt es sich dabei z.B. um das '"ubliche' IEEE 754 64-Bit-Format,
so ist der Wertebereich auf ca. +/-$1.8*10^{308}$ beschr"ankt (siehe Variable
\tty{FLOATMAX}), und die untersten zwei bzw. acht Bytes bei \tty{DT} bzw.
\tty{DO} sind Null.
...
\tty{D1} funktioniert analog zu \ttindex{DB} und \ttindex{DN}, der
Unterschied ist aber, da"s hiermit nur ein einzelnes Bit abgelegt bzw.
reserviert wird.  Diese Anweisung ergibt daher auch nur in Adre"sr"aumen
eine Sinn, die bitweise adressiert werden.  Das ist aktuell nur im
{\tt BITDATA}-Segment des 8051 der Fall, falls die {\tt BITSEGSIZE:1}-Option
benutzt wurde.
...
\tty{DC24} funktioniert analog zu \ttindex{DW} und \ttindex{DD}, legt
aber (Ganzzahl-)Werte mit 24 Bit L"ange ab.
...
\tty{FLT} funktioniert analog zu \tty{DD}, legt jedoch ausschlie"slich
Gleitkomma-Konstanten im IEEE-Format mit einfacher Genauigkeit ab.

%%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\subsection{FLT2, FLT3, FLT4}
...
DEC-eigenen Formaten im Speicher abgelegt, wobei {\tt x} f"ur die vier
unterst"utzten Formate (F, D, G und H) steht.  {\tt FLOAT} ist ein
Alias f"ur {\tt F\_FLOATING}, {\tt DOUBLE} einer f"ur {\tt D\_FLOATING}.
Die Befehle funktionieren ansonsten wie \tty{DD} und \tty{DQ}.

%%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\subsection{DS, DS8, DS16, DS32, DS64}
\ttindex{DS}
\ttindex{DS8}
...
gewordener Leute so fest verdrahtet zu sein, da"s sie
ihn als eingebauten Befehl erwarten.
...
{\em G"ultigkeit: VAX, CR16}

Diese Befehle reservieren Speicher f"ur die angegebene Menge von Datenelementen,
deren Typ im letzten Zeichen des Befehls kodiert ist.  Dementsprechend ist
der insgeamt reservierte Bereich in Bytes bei {\tt BLKB} gleich der Anzahl
der Elemente, bei {\tt BLKW} das doppelte, und bei {\tt BLKO} und {\tt BLKH}
das sechzehnfache.
...
die angegebenen 16-Bit-Werte werden in jeweils zwei aufeinander folgenden
...
{\em G"ultigkeit: TMS340xx}
 
\tty{FIELD} legt im Speicher ein Bitfeld mit einer bestimmten Länge und Wert
ab.  Das zweite Argument ist die L"ange des Feldes (1 bis 32 Bit), das erste
ist der darin abzulegende (Integer-)Wert.  Der erlaubte Wertebereich ist von
der L"ange abh"angig und kann wahlweise vorzeichenlos oder vorzeichenbehaftet
im Zweierkomplement sein.  F"ur ein Bitfeld mit 16 Bit L"ange geht der erlaubte
Wertebereich z.B. von -32768 bis +65535.
...
\subsection{ASCII, ASCIC und ASCIZ}
\ttindex{ASCII}\ttindex{ASCIC}\ttindex{ASCIZ}

{\em\begin{tabbing}
G"ultigkeit: \= ST6, PDP-11, VAX, IMP-16, IPC-16 ({\tt ASCII}) \\
             \> ST6, PDP-11, VAX ({\tt ASCIZ}) \\
             \> PDP-11, VAX ({\tt ASCIC}) \\
\end{tabbing}}

Mit diesen Befehlen k"onnen Stringkonstanten im Speicher
...
mit einem NUL-Zeichen am Ende, und \tty{ASCIC} stellt ihm ein L"angen-
Byte voran.
...
{\em G"ultigkeit: PDP-11, VAX}

Mit diesem Befehl werden Zahlen im gepackten dezimalen Format
(zwei Stellen pro Byte) im Speicher abgelegt.  Als Argument sind
entweder eine (vorzeichenbehaftete) Ganzzahl oder ein String
erlaubt, wobei letzterer nat"urlich nur dezimale Ziffern und optional
ein Vorzeichen am Anfang enthalten darf.  Die Maximalzahl Stellen
ist 31, und das Vorzeichen wird als letzte Stelle angef"ugt, wobei
12 und 13 f"ur Plus bzw. Minus stehen.  Ist die Gesamtzahl aus
Stellen und Vorzeichen ungerade, wird vorne eine Null eingef"ugt.
...
Stellenzahl sind weder Vorzeichen noch eine automatisch eingef"ugte
Null zu Beginn enthalten.
...
Speicher abgelegt werden.  Diese Darstellung war besonders im DEC-Umfeld
f"ur Dateinamen g"angig und packt drei Zeichen in ein 16-Bit-Wort.
{\tt RADIX50} ist keine eingebaute Anweisung, sondern in der Include-
Datei {\tt radix50.inc} als Makro definiert.
...
\tty{ALIGN} mit einem Integer-Ausdruck als Argument erlaubt es, den
...
\tty{EVEN} ist eine Spezialisierung von \tty{ALIGN}.  Der Befehl inkrementiert
den Programmzähler so weit, da"s er ein mehrfaches von 16 Bit ist, also passend
zum Beispiel zur Ablage von Prozessorbefehlen.
...
vollkommen legal.  Wie bei jeder Rekursion mu"s man dabei nat"urlich
...
Ein an ,,Intel-Syntax'' gew"ohnter Programmierer m"ochte die Befehle \tty{PUSH/POP}
...
Infolgedessen sind alle darauf folgenden Labels um zwei zu niedrig,
...
Steueranweisungen (durch geschweifte Klammern als solche gekennzeichnet)
enthalten:
\begin{itemize}
\item{\tty{GLOBALSYMBOLS} bzw. \tty{NOGLOBALSYMBOLS}, um zu steuern,
      ob benutzte Labels f"ur jeden Durchgang automatisch lokal
      sind oder nicht.}
\item{\tty{EXPAND} bzw. \tty{NOEXPAND}}
\item{\tty{EXPIF} bzw. \tty{NOEXPIF}}
\item{\tty{EXPMACRO} bzw. \tty{NOEXPMACRO}}
\item{\tty{EXPREST} bzw. \tty{NOEXPREST}}
\end{itemize}
...
\par
An Steueranweisungen sind die gleichen wie f"ur \tty{IRP} erlaubt.
...
assembliert, bis der Ausdruck logisch falsch wird.  Im Extremfall kann
...
sq       set     cnt*cnt
        endm
\end{verbatim}
An Steueranweisungen sind die gleichen wie f"ur \tty{IRP} und
\tty{REPT} erlaubt.
\par
...
\par
Der Ergebnistyp kann bei Funktionen vom Typ der Argumente abh"angen.
...
\ttindex{IFSYMEXIST}\ttindex{IFNSYMEXIST}
\ttindex{IFUSED}\ttindex{IFNUSED}
\ttindex{IFEXIST}\ttindex{IFNEXIST}
\ttindex{IFB}\ttindex{IFNB}
\begin{itemize}
\item{\tty{IFDEF} $<$Ausdruck$>$ : wahr, falls der Ausdruck keine Symbole
      enth"alt, die im bis zu diesem Punkt assemblierten Quellcode (noch) nicht
      definiert wurden.  ACHTUNG! Bei Vorw"artsdeklarationen liefert diese Abfrage
      in allen Durchl"aufen  ein negatives Ergebnis!}
\item{\tty{IFNDEF} $<$Ausdruck$>$ : Umkehrung zu \tty{IFDEF}}
\item{\tty{IFSYMEXIST} $<$Symbol$>$ : wahr, falls dieses Symbol in der Symboltabelle
      existiert.  ACHTUNG! Bei Vorw"artsdeklarationen liefert diese Abfrage im
      ersten Durchlauf ein negatives Ergebnis, aber ein positives in allen
      weiteren!}
\item{\tty{IFNSYMEXIST} $<$Symbol$>$ : Umkehrung zu \tty{IFSYMEXIST}}
\item{\tty{IFUSED} $<$Symbol$>$ : wahr, falls das Symbol bisher mindestens einmal
      benutzt wurde.  Siehe Abschnitt \ref{sect_symused} f"ur Ausnahmen davon.}
...
\subsection{LISTING}
\ttindex{LISTING}\label{SectListingCmd}
...
Der Startwert kann "uber die gleichnamige Kommandozeilen-Option
gesetzt werden.
...
\begin{verbatim}
        PRTINIT <String>
\end{verbatim}
die Zeichenfolge angibt, die vor Beginn des Listings an das Ausgabeger"at
...
Beispiel:
\par
Bei dem bereits oben angesprochenen Epson-Drucker soll eine Titelzeile
...
Eine {\tt RADIX}-Anweisung "ubersteuert einen eventuell gesetzten
{\tt -radix}-Schalter auf der Kommandozeile.
...
wird damit explizit als lokal angek"undigt.  \asname{} wird bei Zugriffen
...
Im Gegensatz zu \tty{PUBLIC} und \tty{GLOBAL} darf \tty{FORWARD} auch
au"serhalb von Sektionen verwendet werden.  In diesem Fall bleibt die
Funktion erhalten, da"s in der Symboltabelle ein 'Platzhalter-Eintrag'
angelegt wird.  Dieser enth"alt zwar keinen Wert, und die Bezugnahme auf
so einen Eintrag wird wie der Zugriff auf ein (bisher) nicht definiertes
Symbol behandelt.  Es wird in so einem Fall aber das Used-Flag an diesem
Platzhalter-Eintrag gesetzt.  Eine \tty{FORWARD}-Deklaration eines
Symbols kann daher n"utzlich sein, wenn ein Symbol per \tty{SYMUSED} auf
Benutzung gepr"uft werden soll, und es Referenzen vor seiner eigentlichen
Definition im Quellcode gibt.
...
Schritt eine Endung \tty{.INC} angenommen.  Erst wenn keine solche Datei
existiert, oder der angegebene Name bereits einen Punkt und damit eine Endung
enth"alt, wird nach einer Datei genau mit dem anggegebenen Namen gesucht.
\par
Der Assembler versucht als erstes, die Datei in dem Verzeichnis
...
\par
Es gelten die gleichen Regeln bez"uglich Suchpfaden und impliziten Suffixen
wie bei \tty{INCLUDE}.
...
,,herum stochern'' zu m"ussen.  Die im Beispiel gezeigte \tty{IF-}
...
Aus historischen Gr"unde ist das {\tt BITDATA}-Segment im Default
{\em nicht} bitweise organisiert, d. h. eine Anweisung wie
\begin{verbatim}
flag db ?
\end{verbatim}
setzt den Programmz"ahler nur um eins weiter, und nicht um acht.  Es ist
quasi implizit vereinbart, dass Bytes in Wirklichkeit nur Bits sind - und
wer nicht nur Speicher reserviert, sollte nur Null und eins als Werte
ablegen. Dieses etwas unintuitive Verhalten ist der Tatsache geschuldet,
da"s fr"uhere Versionen von \asname{} schlicht keine Adre"sr"aume mit
einer Wortbreite unterst"utzten, die nicht 8, 16 oder 32 Bit war.  Und 
als es dann m"oglich war, h"atte eine harte Umstellung dazu gef"uhrt, da"s
existierende Programme nicht mehr wie vorher funktionieren.

Wer ein in Bits organisiertes {\tt BITDATA}-Segment haben m"ochte, mu"s
dieses daher explizit anfordern, indem bei der Auswahl des Ziels eine
Option angegeben wird:
\begin{verbatim}
	cpu     8051:bitsegsize=1
\end{verbatim}
Des weiteren ist zu beachten, da"s bei der Ausgabe von in diesem Segment
abgelegten Daten mittels P2BIN oder P2HEX ein einzelnes Bit nach wie vor
ein komplettes Byte belegt.
...
\par
Der beim Z80-Target definierte 'generische Sprungbefehl' {\tt J} ist
bei aktivierter Z80-Syntax ebenfalls verf"ugbar, da der 8080/8085
jedoch keine relativen Spr"unge unterst"utzt, wird {\tt J} immer
in {\tt JP} "ubersetzt.
...
\section{Z80, Z18, Z280, Z380}

Der Z80 kennt zwei Arten von Spr"ungen: relativ ({\tt JR}) erlaubt Sprungdistanzen
von -128 bis +127 Byte, w"ahrend man mit absoluten Spr"ungen ({\tt JP}) den
kompletten Adre"sraum erreicht.  \asname{} unterst"utzt einen Pseudobefehl
{\tt J}, der anhand der Zieladresse und der Sprungbedingung automatisch die
k"urzestm"ogliche Variante w"ahlt (relative Spr"unge sind nicht f"ur alle
Bedingungen definiert).  Dies gilt auch f"ur alle vom Z80 abgeleiteten Targets,
wie Z80UNDOC, Z180, Z280, RABBIT2000 und LR35902.

F"ur den Z380 wird {\tt J} ebenfalls angeboten, da der Z380 jedoch gr"o"sere
Sprungdistanzen unterst"utzt, wird erst auf absolute Spr"unge ausgewichen,
wenn die g"o"stm"ogliche Sprungdistanz (+/- 8 MByte) nicht mehr ausreicht.

Generell sollte dieser Befehl mit Bedacht eingesetzt werden, da {\tt JR} und
{\tt JP} nicht g"anzlich funktionsgleich sind: Code, der ausschlie"slich
relative Spr"unge benutzt, kann positionsunabh"angig ausgef"uhrt werden,
w"ahrend absolute Spr"unge einer Relozierung bed"urfen.
...
\par
Wie auch beim Z280/Z380 und eZ80 ist es m"oglich, die Byte-H"alften von IX und IY
...
Kompliziert wird die Sache dadurch, da"s sowohl die mit LW eingestellte
Operandengr"o"se, als auch die L"ange von immediate- und absoluten
Operanden, f"ur einzelne Befehle mit den Pr"afixen \tty{DDIR W/LW/IB/IW}
\asname{} merkt sich die Verwendung solcher Befehle und ber"ucksichtigt
sie f"ur den n"achsten Befehl automatisch mit.  Dabei sind die folgenden
F"alle zu unterscheiden:

Ging gar kein \tty{DDIR}-Befehl voraus bzw. einer, der keine explizite
Operandenl"ange enthielt (also weder \tty{IN}, \tty{IB}, noch \tty{IW}), so wird die
Operandenl"ange automatisch festgelegt.  Ein eventuell erforderlicher
\tty{DDIR}-Pr"afix wird automatisch eingef"ugt.  Ein einzelnes
\begin{verbatim}
        LD      BC,12345678h
\end{verbatim}
wird also automatisch um ein f"uhrendes \tty{DDIR IW} erg"anzt.  In diesem
Fall jedoch:
\begin{verbatim}
        DDIR    LW
        LD      BC,12345678h
\end{verbatim}
wird der \tty{DDIR}-Pr"afix automatisch um ein \tty{IW} erg"anzt,
effektiv wird also der Code f"ur
\begin{verbatim}
        DDIR    LW,IW
        LD      BC,12345678h
\end{verbatim}
...
Explizite \tty{DDIR}-Pr"afixe mit einem \tty{IN}, \tty{IB}, oder
\tty{IW} k"onnen genutzt werden, um eine bestimmte Operandenl"ange
zu erzwingen.  Zu lange Operanden f"uhren zu einer Fehlermeldung,
k"urzere werden mit Nullen auf die geforderte L"ange gebracht.  Ein
\begin{verbatim}
        DDIR    IB
        LD      BC,12345678h
\end{verbatim}
f"uhrt also zu einer Fehlermeldung, weil die Konstante nicht in 24
Bit hineinpasst.  Umgekehrt wird hier
\begin{verbatim}
        DDIR    LW,IW
        LD      BC,345678h
\end{verbatim}
...
\begin{itemize}
\item{{\em Nicht segmentiert}: Der Adre"sraum ist auf 64 KByte
...
\item{Unter TAS900 k"onnen Integer-Konstanten sowohl in C-Notation (mit
...
\hline
\tty{\$NOLIST}      & \tty{LISTING OFF}         & Assembler-Listing ausschalten \\
\hline
\tty{\$LIST}        & \tty{LISTING ON}          & Assembler-Listing einschalten \\
\hline
...
Als National die CR16-Architektur mit dem CR16B auf 2 MByte Adre"sraum
erweiterte, war die Kompatibilit"at zum Vorg"anger CR16A ein Thema.  CR16B
hat deshalb ein 'Kompatibilit"atsbit', in dem er vollst"andig bin"ar
aufw"artskompatibel ist.  Der Preis daf"ur ist, da"s diverse Sprungbefehle
weiterhin nur die ersten 128 Kbyte des Adre"sraumes erreichen k"onnen.  Im
,,Large Model'' "andert sich die bin"are Kodierung dieser Befehle, so da"s
die vollen 2 MByte erreicht werden k"onnen.  Ob man beim CR16B Code f"ur
das kleine oder gro"se Speichermodell erzeugen will, w"ahlt man zusammen
mit der \tty{CPU}-Anweisung:

\begin{verbatim}
   cpu cr16b:model=0
\end{verbatim}

w"ahlt das kleine Modell,

\begin{verbatim}
   cpu cr16b:model=1
\end{verbatim}

w"ahlt das gro"se Modell.  Letzteres ist auch der Default, wenn diese Option
weggelassen wird.
...
Der uCOM-43-Befehlssatz enth"alt einen Befehl {\tt CZP}, der es erlaubt, einen
Unterprogrammaufruf auf eine der Adressen 0, 4, 8, 12...60 mit einem einzigen
Instruktions-Byte zu machen.  Leider ist die vorhandene Dokumentation dar"uber
unklar, ob der Befehl im Quelltext die Zieladresse (also einen durch vier
teilbaren Wert) oder den im Befehl enthaltenen Vektor (also eine Zahl von 0 bis
15) erwartet.  Im Moment betreibt der Assembler ein wenig 'Kaffeesatzleserei':
\begin{itemize}
\item{Falls das Argument gr"o"ser als 15 ist, mu"s es sich um eine Adresse
      handeln, ansonsten:}
\item{Falls das Argument kein mehrfaches von vier ist, mu"s es sich um einen
      Vektor handeln, ansonsten:}
\item{Falls das Argument Null ist, ist keine Unterscheidung notwendig, ansonsten:}
\item{Falls das Argument ein Symbol aus dem {\tt CODE}-Segment ist, z.B. ein Label,
      mu"s es sich um eine Adresse handlen, ansonsten:}
\item{ist es ein Vektor.}
\end{itemize}
Die letzten beiden F"alle treffen nur auf die Werte 4, 8 oder 12 zu, und weil die
Entscheidung nicht intuitiv ist, wird bei der Entscheidung f"ur einen Vektor
eine Warnung ausgegeben.  F"ur den Fall, da"s wirklich die Adresse 4, 8 oder 12
gemeint ist, definiere man den Wert als Symbol im Code-Segment:
\begin{verbatim}
dest   label  4
       .
       .
       ...
       czp dest
\end{verbatim}
F"ur den Fall, da"s ein Vektor gemeint ist, und die Warnung st"ort, kann man die
Warnung folgender ma"sen unterdr"ucken:
\begin{verbatim}
vector equ    4
       .
       .
       ...
       endexpect
\end{verbatim}
Ich gestehe ein, da"s diese Situation nicht 100\% befriedigend ist.  Falls
jemand Informationen "uber das Verhalten des Original-NEC-Assemblers hat,
w"are ich f"ur eine Nachricht dankbar.
...
\newcommand{\headid}[2]{#1 & #2
  \ifnum\value{headidcounter}>1 \\ \fi
  \ifnum\value{headidcounter}<2 & \fi
  \stepcounter{headidcounter}
...
die mit Adressen von 0,1,2,...65535 adressiert werden.  Granularit"aten
kleiner als 8 Bit werden mit Werten von \$f9..\$ff kodiert.  Falls das
kleinste adressierbare Element ein Bit ist, ist die Granularit"at \$ff,
ist sie ein Nibble, ist die Granularit"at \$fc.
\par
...
indem man sie mit einem hintan gestellten \tty{h}, einem voranstehenden
...
CMD an den Programmnamen ergibt, z.B. PBINDCMD f"ur PBIND), Optionen
...
PBIND ist ein Programm, mit dem man die Records mehrerer Codedateien
...
Auf diese Weise kann PBIND auch dazu verwendet werden, um eine Codedatei
in mehrere aufzuspalten.
\par
Die allgemeine Syntax von PBIND lautet
\begin{verbatim}
   PBIND [Optionen] <Quelldatei(en)> [Zieldatei]
\end{verbatim}
Wie auch \asname{} betrachtet PBIND alle nicht mit einem +, - oder / eingeleiteten
Parameter als Dateiangaben.  Falls keine \tty{-o} Option angegeben wurde, wird
die letzte davon als Zieldatei angenommen.
Alle anderen Dateiangaben bezeichnen Quellen, diese Angaben d"urfen
auch wieder Jokerzeichen enthalten.
\par
An Optionen definiert PBIND momentan nur zwei:
\begin{itemize}
\item{\tty{o $<$Datei$>$}: spezifiziert die Ausgabe/Zieldatei.}
...
benutzt man PBIND folgenderma"sen:
\begin{verbatim}
   PBIND <Quellname> <Zielname> -f $31
\end{verbatim}
...
P2HEX ist eine Erweiterung von PBIND.  Es besitzt alle
Kommandozeilenoptionen von PBIND und hat die gleichen Konventionen
bez"uglich Quell- und Ziel-Dateinamen.  Im Gegensatz zu PBIND wird
die Zieldatei aber als Hexfile ausgegeben, d.h. als eine Folge von
Zeilen, die den Code als ASCII-Hexzahlen enthalten.
\par
P2HEX kennt zehn verschiedene Zielformate, die "uber den
...
\par
Anders als PBIND erzeugt P2HEX keine leere Datei, wenn nur ein Dateiname
...
\errentry{  10}{Kurzadressierung m"oglich}
               ...
               {bei 680x0-, 6502- und 68xx-Prozessoren k"onnen
                ...
                werden.  Um keine Phasenfehler zu erzeugen, wird zwar der
...
\errentry{  20}{kurzer Sprung m"oglich}
               ...
               {keines}
\errentry{  25}{relativer Sprung m"oglich}
               {Warnung}
               {Spr"unge k"onnen beim Z80 sowohl absolut als auch relativ
                ausgef"uhrt werden.  In diesem Fall wurde ein absoluter
                Sprung angefordert, ein(k"urzerer) relativer w"are aber
                m"oglich.}
               {das Sprungargument}
...
\errentry{ 460}{CASE-Limit passt nicht zur Anzahl der Sprungadressen}
               {Warnung}
               {Die CASE-Anweisung erwartet eine Sprungtabelle mit so vielen
                Eintr"agen, wie das Limit-Argument plus eins angibt.  Limit-Wert
                und Anzahl der Sprungadressen passen nicht zueinander.}
               {das Limit-Argument}
\errentry{ 470}{Befehl als NOP assembliert}
               {Warnung}
               {Der dieser Anweisung eigentlich zugewiesene Opcode wird anderweitig
                verwendet.  Da die Anweisung ohnehin nichts bewirkt, wird sie stattdessen
                als NOP assembliert.}
               {keines}
\errentry{ 480}{Argument wird als Vektor behandelt}
               {Warnung}
               {Anhand des Arguments kann nicht zweifelsfrei entschieden werden, ob
                es sich um eine Adresse oder einen Vektor handelt.  Da das Argument eine
                reine Zahl und keinem Segment zugeordnet ist, wird es als Vektor behandelt.}
               {das fragliche Argument}
\errentry{ 490}{Zu grosse Integer-Konstante wird als Gleitkomma interpretiert}
               {Warnung}
               {Das Argument ist zwar eine syntaktisch korrekte Ganzzahl-Konstante, ihr
                Wert ist aber au"serhalb dessen, was intern als Ganzzahl dargestellt
                werden kann.  Die Zahl wird deswegen als Gleitkomma-Zahl gespeichert.}
               {das fragliche Argument}
\errentry{ 500}{Code-Erzeugung ausserhalb des CODE-Segments}
               {Warnung}
               {Maschinencode kann nur innerhalb des CODE-Segments erzeugt werden. Die
                anderen Adressr"aume verf"ugen nicht "uber die daf"ur erforderliche
                Wortbreite.}
               {keines}
\errentry{ 510}{Befehl wird SP "uberschreiben}
               {Warnung}
               {Dieser Befehl benutzt neben dem explizit genannten
                noch das Folgeregister f"ur das doppelt lange Ergebnis.
                Das Folgeregister ist in diesem Fall aber der
                Stackpointer.}
               {das Zielargument der Anweisung}
...
\errentry{1030}{reservierter Symbolname}
               {Fehler}
               {Der Symbolname ist zwar an sich g"ultig, dieser spezielle
                Name ist aber f"ur andere Zwecke reserviert, und kann nicht
                f"ur selbst definierte Symbole verwendet werden.}
...
\errentry{1112}{Kann Argument nicht in Teile aufspaltenl}
               ...
               {Bei bestimmten Prozessoren (z.B. DSP56000)
                ...
                fehlgeschlagen.}
               {keines}
\errentry{1115}{unpassende Optionszahl}
               {Fehler}
               {Die bei diesem Befehl angegebene Zahl
	        von Optionen liegt nicht in dem f"ur diesen
                Befehl erlaubten Bereich.}
               {keines}
\errentry{1116}{unbekannte Option}
               {Fehler}
               {Eine Option dieses Namens existiert nicht.}
               {die fragliche Option}
\errentry{1117}{doppelte Option}
               {Fehler}
               {Optionen d"urfen nicht mehfach aufgef"uhrt werden.}
               {die fragliche Option}
\errentry{1118}{nicht unterst"utzte Optionsliste}
               {Fehler}
               {Optionen sind in dieser Kombination nicht verwendbar.}
               {die fragliche Optionsliste}
...
\errentry{1153}{Ganzzahl erwartet, aber Register bekommen}
               ...
               {das fehlerhafte Argument}
\errentry{1154}{String zu lang}
               {Fehler}
               {Dieser String ist zu lang, als da"s er mit vorangestelltem
                L"angen-Byte dargestellt werden k"onnte.}
               {das fehlerhafte Argument}
...
\errentry{1322}{keine Zweierpotenz}
               ...
               {Hier sind nur Zweierpotenzen (1,2,4,8...)
                als Wert erlaubt}
               {der fragliche Wert}
\errentry{1323}{ung"ultige Dezimalstelle}
               {Fehler}
               {Ein String als Argument zu {\tt PACKED} darf nur
                die Ziffern 0 bis 9 enthalten, sowie ein Plus- oder
                Minuszeiche nzu Anfang.}
               {das fragliche Argument}
\errentry{1324}{Dezimalstring zu lang}
               {Fehler}
               {Eine gepackte Dezimalzahl darf maximal 31 Stellen
                lang sein.}
               {das fragliche Argument}
...
\errentry{1437}{Ung"ultiges Prozessorregister}
               {Fehler}
               {Ein Prozessorregister dieses Namens existiert nicht.}
               {das fragliche Register}
\errentry{1438}{Ung"ultiges Basisregister}
               {Fehler}
               {Dieses Register darf an dieser Stelle nicht als Basis benutzt
	        werden.}
               {das fragliche Register}
\errentry{1439}{Ung"ultiges Indexregister}
               {Fehler}
               {Dieses Register darf an dieser Stelle nicht als Index benutzt
	        werden.}
               {das fragliche Register}
\errentry{1440}{Ung"ultiges Steuerregister}
               {Fehler}
               {Das bei z.B. \tty{MOVEC} benutzte
	        Kontrollregister  kennt der mit CPU gesetzte Prozessor
	        (noch) nicht.}
               {keines}
\errentry{1441}{unbekannter Vektor}
               {Fehler}
               {Ein Vektor dieses Namens existiert nicht.}
               {das fragliche Vektor-Argument}
\errentry{1442}{Register im aktuellen Ausf"uhrungsmodus nicht zugreifbar}
               {Fehler}
               {Dieses Register ist nur im Kernel-Modus zugreifbar.}
               {das fragliche Register}
\errentry{1443}{Register darf im aktuellen Ausf"uhrungsmodus nur gelesen werden}
               {Fehler}
               {Dieses Register ist nur im Kernel-Modus schreibbar.}
               {das fragliche Register}
...
\errentry{1480}{ung"ultiges IF-Konstrukt}
	        ...
	        oder \tty{SWITCH}-Konstrukt stimmt nicht.}
               {keines}
\errentry{1482}{FORWARD-Anweisung mu"s sich auf aktuelle Sektion beziehen}
               {Fehler}
               {\tty{FORWARD}-Direktiven d"urfen sich nicht auf Symbole in
                einer anderen (h"oheren) Sektion beziehen.}
...
\errentry{1490}{falsche Argumentzahl f"ur Funktion}
               ...
               {Die Anzahl der Argumente f"ur eine
                ...
                Anzahl "uberein.}
               {keines}
\errentry{1491}{doppelter Funktions-Argumentenname}
               {Fehler}
...
\errentry{1520}{nur ON/OFF erlaubt}
               ...
               {Dieser Pseudobefehl darf als Argument nur \tty{ON}
                oder \tty{OFF} haben.}
               {keines}
\errentry{1521}{ung"ultiger LISTING-Wert}
               {Fehler}
               {Als Argument f"ur \tty{LISTING} sind nur die Werte
                \tty{ON}, \tty{OFF}, \tty{NOSKIPPED} oder \tty{PURECODE}
                erlaubt.}
               {das fragliche Argument}
...
\errentry{1805}{EXITM au"serhalb eines Makrorumpfes}
               ...
               {\tty{EXITM} bricht die Expansion von
                ...
                aufzurufen.}
               {Der fragliche EXITM-Befehl}
\errentry{1806}{ENDM au"serhalb eines Makrorumpfes}
               {Fehler}
               {\tty{ENDM} schlie"st die Definition eines Makros oder einen
                \tty{REPT/IRP/WHILE}-Block ab.  Au"serhalt davon ergibt
                er keinen Sinn.}
               {Der fragliche ENDM-Befehl}
...
\errentry{2270}{Seite nicht zugreifbar}
               ...
               {Die Zieladresse liegt auf einer Speicherseite, die aktuell nicht
                zugreifbar ist.}
               {keines}
\errentry{2271}{aktueller Programmz"ahler in nicht-zugreifbarer Seiter}
               {Fehler}
               {Die aktuell assemblierte Instruktion liegt auf einer physischen
                Adresse, die nicht in den logischen Adre"sraum der CPU gemappt
                ist.  Dies ist ein Problem bei PC-relativer Adressierung, weil
                f"ur die Berechnung der Distanz die logische Adresse gebraucht
                wird.}
               {keines}
...
\errentry{2320}{kein Target gesetzt ('CPU ...' oder '-cpu ...', um eines zu setzen)}
               ...
               {keines}
\errentry{2330}{Ung"ultige Displacement-L"ange}
               {Fehler}
...
\errentry{10002}{Listingschreibfehler}
               {fatal}
               {Beim Schreiben des Assembler-Listings ist ein
                Fehler aufgetreten.}
               {Beschreibung des E/A-Fehlers}
...
\errentry{10010}{ung"ultiger Platzhalter in Listing Pro-Zeile-Pr"afix Format}
               {fatal}
               {Als Platzhalter sind nur \%i, \%n oder \%a zul"assig.}
               {das ung"ultige Format}
\errentry{10011}{zu oft verwendeter Platzhalter in Listing Pro-Zeile-Pr"afix Format}
               {fatal}
               {Die Platzhalter \%i und \%n d"urfen jeweils nur maximal dreimal
                im Formatstring enthalten sein.  Gen"ugt das nicht?}
...
\item[159]{Papierende\\
           ...
           Drucker umgeleitet.  Assembler-Listings k"onnen seeehr lang
           sein...}
...
\section{16-Bit-Befehle per Makro}
...
diesem kann das f"uhrende Hash-Zeichen dann auch entfernt werden, um den eigentlichen
...
\begin{verbatim}
        ...
        subd    $12,x                   ; indexed
\end{verbatim}
Jetzt ist es aber so, dass man nicht nur 16-bittig subtrahieren m"ochte.  Man
...
\newcommand{\pcsymname}{Symbol f"ur Programmz"ahler}
...
Bolsch, Rolf Buchholz, Bernd Casimir, Nils Eilers, Jeff Epler, Gunther Ewald,
Daniel E. Germann, Michael Haardt, Stephan Hruschka, Fred van
Kempen, Peter Kliegelh"ofer, Ulf Meinke, Udo M"oller, Matthias
Paul, Moshe Piekarski, Norbert Rosch, Curt J.  Sampson, Steffen Schmid,
Leonhard Schneider, Ernst Schwab, Michael Schwingen, Oliver Sellke,
Christian Stelter, Patrik Str"omdahl, Tadashi G. Takaoka, Oliver
Thamm, Thorsten Thiele, Leszek Ulman, Rob Warmelink, Andreas
Wassatsch, John Weinrich,
\par
...
\item{Version 1.39:
      ...
      \item{Variable \tty{MOMPASS}, um die Nummer des augenblicklichen Durchganges
            abfragen zu k"onnen;}
      \item{Kommandozeilenschalter \tty{t}, um einzelne Teile des Assembler-Listings
...
\item{Version 1.40:
      ...
      \item{Escapesequenzen sind nun auch in in ASCII geschriebenen
            Integer-Konstanten zul"assig(p1);}
...
\subsection{Von \asname{} genutzte Module}
...
\item{Eine Hintert"ur, falls die dar"uber liegende Entwicklungsumgebung die
...
werden.  Dies ist einmal die Intel-Gruppe mit der \tty{DB..DO}-Gruppe,
...
\item{\tty{ListGrans} : Hiermit kann wieder f"ur alle Segmente getrennt
      festgelegt werden, in was f"ur Gruppen die Bytes im Assembler-Listing
...
\item{\tty{ConstMode} : Diese Variable kann die Werte \tty{ConstModeIntel},
      ...
      welcher Form Zahlensysteme bei Integer-Konstanten spezifiziert werden
