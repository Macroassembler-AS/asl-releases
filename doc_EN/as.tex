...
{\em CompactRISC} is a registered trademark of National
 Semiconductor (now a part of Texas Instruments).
...
{\em Linux} is a registered trademark of Linus Torvalds.
...
\item{AT\&T WE32100/we32200 (''Bellmac32'')}
...
\item{Zilog Z80 (including undocumented instructions), Z180, Z280, Z380, eZ80}
...
with a width of 32, 64, or 128 bits for integer values, 80 or 64 bits for floating
...
\item{{\tt list-unknown-values} resp. {\tt no-list-unknown-values}: enables
      or disables displaying question marks for values unknown in the
      first pass.}
\item{\label{radix}\tty{RADIX}: This option changes the default number system
      to a value between 2 and 36, thereby overriding the default of 10.
      The value given by this switch can itself be overridden in the program
      by the statement odf same name.}
\item{\tty{listing}: Defines whether the listing of assembled code shall
      contain everything, or whether certain parts excluded by conditional 
      assembly shall be omitted.  Valid agruments to this option are ther same
      as described in \ref{SectListingCmd}.}
...
\item{\tty{intsyntax}: this switch allows to augment or reduce the list of
      allowed integer constant syntaxes right from the beginning of the
      program.  See section \ref{SectINTSYNTAX} for a list of allowed
      arguments.}
...
\item{\tty{extracomments}: This switch by default enables the usage of
      alternate methods to write comments (see section \ref{SectEXTRACOMMENTS}).}
...
\item{{\tt maxinclevel [n]}: instructs the assembler to terminate
      assembly if the include nesting level exceeds the given limit
      (default is 200).}
\item{{\tt maxsympass [n]}: defines up to which pass unresolved forward
      references are allowed.  Changing from the default value (1) to
      a higher one may be needed in special cases, like chained
      forwardreferences.}
...
\item{{\tt wimplicit-sign-extension} resp. {\tt wno-implicit-sign-extension}: instructs
...
Everything following a semicolon is regarded as a comment and will
not be regarded any further during assembly.  Depending on its position
in the source line, the line may only contain a label and/or a mnemonic,
or it may even cotain nothing but a comment.  A few targets support
additional ways to mark comments as such:
\begin{itemize}
\item{Padauk controllers allow to use a double forward slash instead of
      a semicolon, as it is known from C++.}
\item{If the \tty{EXTRACOMMENTS} option is enabled (see \ref{SectEXTRACOMMENTS})
      on 68xx targets, the rest of the line is regarded as comment if
      the label or mnemonic begins with an asterisk.}
\item{The \tty{EXTRACOMMENTS} option on 68xx targets also enables the
      usage of end-of-line comments.  An instruction's argument list
      must not contain any spaces, and everything after the first
      space is treated as comment.}
\end{itemize}
...
also in the number system defined by the list radix.  Depending on the
processor target and current address space, the values are formatted with
different length.  In the simplemost case, the code may be shown as a
sequence of 8 bit bytes.  Words of 16 or 32 bits may also be used, in
case the address space is not byte-addressable, or the target processor's
instruction words are longer than one byte.  For instance, the address
space of the 68000 or the PDP-11 is byte-addressable, their machine instructions
however have a length of 16 bits.  There are even a few cases when the
word length is not a multiple of 8 bit, like 12 bits for the PDP-8.

If forward references are used, the final values for parts of the
machine code cannot be listed in the first pass. \asname{} will then
make an assumption about the value that permits further assembly.  For
a (growing) number of targets, these fields will be displayed with
question marks.  The following targets currently support this feature:
\begin{itemize}
\item{68xxx/Coldfire}
\item{DSP56000/56300}
\item{PowerPC}
...
listing, this feature may be disabled via the \tty{-no-list-unknown-values}
command line switch.
...
HAS64         & internal integer arithmetic uses at leat 64 bitt \\
...
The functions \tty{DEFINED} resp. \tty{SYMEXIST} allow to check whether a
symbol is defined or not.  The difference between them: \tty{SYMEXIST} returns
'false' for a forward declaration in pass one, and 'true' in subsequent passes.
In contrast, \tty{DEFINED} consistently returns 'false' for forward declarations
in all passes - the meaning is 'defined up to this point in the source'.

\label{sect_symused}
The function \tty{SYMUSED} allows to query whether a symbol has been
used at least once up to this point.  This includes references made
to a symbol prior to its definition, i.e. forward references, in case
one of these conditions is fulfilled:
\begin{itemize}
\item{Both the forward reference and the definition are done in the
      global symbol name space, i.e. outside of expanded macro bodies
      and sections.}
\item{The forward reference is done inside a section, but outside of
      an expanded macro body, and the definition is also done outside
      of an expanded macro body, and either in the same section, one
      of its parent sections, or globally.}
\item{The forward reference is done inside an expanded macro body, and
      the definition is done in the same macro body.}
\end{itemize}
If a symbol shall be tested for usage, and if there are usages that
do not match any of these conditions, there is still the option to
announce the existence of a symbol via a \tty{FORWARD} declaration.
This effectively creates an 'empty' entry in the symbol table, that
can hold the usage flag even before actual definition of the symbol.

\tty{DEFINED} not only accepts simple symbol names, but also complete
formula expressions.  In this case, the return value is 'true' only if the
expression does not contain any symbols that are undefined by the rule
...
\begin{itemize}
\item{If a {\tt RADIX} statement was given, use the number system
      given by it, otherwise:}
\item{If a {\tt -radix} command line switch was given, use the number
      system given by it, otherwise:}
\item{Use decimal (base 10).}
\end{itemize}
...
In addition to the operators, \asname{} defines another line of
...
\hline
\caption{Functions Predefined by \asname{} - Part 1 (Integer and
...
\tty{FSIZE} returns the size of a file in bytes.  The file is
searched for both in the current directory, and relative to
the directory of the file currently being assembled.  Opposed
to \tty{INCLUDE} and \tty{BINCLUDE}, the include path is
not used to find the file!
...
{\em valid for: PALM, 8008/8080/8085/8086, XA, Z80, Z8000, 320C2x/5x, TLCS-42/47, AVR,
     F8, IMP-16}
...
{\em valid for: 680x0, PowerPC, PALM, M*Core, XGate, H8,\\
...
On PDP-11 and TMS340xx, \tty{REG} may additionally be used without a name in the
label field.  It then expects a single \tty{ON} or \tty{OFF} as argument
and enables or disables the built-in register aliases:
\begin{itemize}
...
\hline
\multicolumn{11}{|l|}{$^{2}$ As Z180 and Z280 are only capable of logically addressing 64 KBytes,}\\
\multicolumn{11}{|l|}{   the full address space can only be used via \tty{PHASE} instructions}\\
\multicolumn{11}{|l|}{   or by setting proper \tty{ASSUME} values for MMU registers.}\\
\hline
...
\hline
\multicolumn{11}{|l|}{$^{11}$ Only the first 128 kbytes are usable for executable code.} \\
\hline
...
%%-----------
\begin{cpulist}
   6809/6809UNDOC/6309 and 6805/68HC(S)08
\end{cpulist}
...
lot of new instructions (see \cite{Kaku}).  6809UNDOC enables the
undocumented {\tt HCF} instruction.
%%-----------
\begin{cpulist}
...
\end{cpulist}
The third generation of the WE32xxx processor features sixteen
additional CPU registers, and along with them, new addressing
modes like indexed and auto-increment/decrement.  Furthermore,
new instructions were added.
%%-----------
\begin{cpulist}
   CP-3F, LP8000, M380
\end{cpulist}
...
\end{cpulist}
The TMS34020 supports a full 4 Gbit address space, additional
machine instructions, and a coprocessor interface.
%%-----------
\begin{cpulist}
...
%%-----------
\begin{cpulist}
   CR16A $\rightarrow$ CR16B $\rightarrow$ CR16
\end{cpulist}
CR16B extends the address from 256 KBytes to 2 MBytes and provides a
couple of new instructions.  CR16C again extends it to 16 MBytes, it
is however only source and no longer binary upward compatible to its
predecessors.
%%-----------
\begin{cpulist}
   F3850, MK3850, \\
...
iso650 is an FPGA implementation of the uPD650.  It differs from
the 'original' by an additional NAND instruction, and an address
space extended from 2000 to 4096 bytes.
%%-----------
\begin{cpulist}
...
{\em
\begin{itemize}
\item{SUPMODE valid for: 680x0, NS32xxx, CR16C, PDP-11, i960,
      TLCS-900, SH7000, i960, 29K, Z280, XA, PowerPC, M*Core,
      V60, and TMS9900}
\item{FPU valid for: 680x0, NS32xxx, 80x86, WE32xxx}
...
\subsection{ACCMODEresp. EXECMODE}
\ttindex{ACCMODE}
\ttindex{EXECMODE}

{\em
\begin{itemize}
\item{valid for: VAX (ACCMODE),\\
      Bellmac32 (EXECMODE)}
\end{itemize}
}

VAX and WE32xxx not only know a user and supervisor mode, they supports four privilege
levels of this type.  Going from more to less rights, these are named Kernel,
Executive, Supervisor, and User.  The {\tt ACCMODE} resp. {\tt EXECMODE} instruction informs the
...
{\em valid for: TMS34020}

This instruction defines the default id (address) to be inserted into
coprocessor instructions if no explicit id was given.  The default for this
value itself is zero.  Allowed values range from zero to seven.
...
The original Motorola assemblers have the feature to regard the rest of a
source line as comment if the label or mnemonic begin with an asterisk.
\asname{} normally does not support this, it may however be enabled via a
\begin{verbatim}
  extracomments on
\end{verbatim}
directive.  This may simplify porting code written for another assembler.

This mode also enables the use of so-called 'end-of-line' comments: an
instruction's argument list ends with the first (non-quoted) space, and
everything thereafter is regarded as comment.

A command line argument option of same name allows to enable this feature
right from the beginning.  The current setting may be read from a variable
of same name.
...
The Z180 contains a built-in MMU which maps the CPU core's ''logical''
...
{\tt BBR}.  Similar to the 68HC11K4, \asname{} will perform automatic
translations of physical to logical addresses, both for absolute addresses
and source and target of relative branches.  It is also possible to access
the mapping tables via the {\tt phys2cpu()} and {\tt cpu2phys()} functions.

%%. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

\subsubsection{Z280}

The Z280's I bit in the Trap Control Register allows to define whether I/O
accesses are allowed in user mode or not.  If \asname{} has been told via
\begin{verbatim}
   assume i:1
\end{verbatim}
that they are forbidden, a warning is issued if I/O instructions are used in
user mode.

Furthermore, the Z280 contains a built-in MMU to translate the CPU's 64K
address space to the 16 Mbytes physically addressable.  Its operation is
controlled via the MMU master control register ({\tt MMUMCR}), and 32
page descriptor registers ({\tt UPD0...UPD15}, {\tt SPD0...SPD15}).  The
value of all these registers can be communicated to \asname{} via
{\tt ASSUME} statements.

The following bits of {\tt MMUMCR} are regarded:
\begin{itemize}
\item{Bit 15 (UTE): User Mode Translate Enable}
\item{Bit 14 (UPD): User Node Program/Data Separation Enable}
\item{Bit 11 (STE): System Mode Translate Enable}
\item{Bit 10 (SPD): System Mode Program/Oata Separation Enable}
\end{itemize}

The following bits of {\tt UPDn/SPDn} are regarded:
\begin{itemize}
\item{Bit 3 (V): Valid Bit}
\item{Bits 15...4 resp. 15...5: page frame address, i.e. bits 23...12
      resp. 23...13 of the page address in physical address space.}
\end{itemize}

Depending on register values, there may be address translation
in system and/or user mode (STE and/or UTE set), or no translation
at all (both STE and UTE cleared).  The current setting of {\tt SUPMODE}
therefore also has an influence on the current mapping tables.  If
enabled, mapping for code and data accesses may be common or different
(UPD/SPD cleared or set).
...
\begin{itemize}
\item{In Z80 mode, the BC, DE, HL, IX, IY, and SP registers are 16 bits wide,
      and only the 64K page defined by the MBASE register is addressable.
      For instructions with an absolute address or a non-8-bit immediate value
      as argument, two bytes are fetched.}
\item{In ADL mode, the mentioned registers are 24 bits wide, and the whole
      16 MByte address space can be addressed.  For instructions with an absolute
...
The CR16C may operate in an alternate mode that limits the size of registers
\tty{R12...SP} to 16 bits, to provide better architectural upward compatibility to
its predecessors.  The mode may be activated via the \tty{SR} bit in the
\tty{CFG} register, and has the following additional effects on the programming
model:
\begin{itemize}
\item{Additional 16 bit register pairs are available: \tty{(R13\_L,R12\_L)},
      \tty{(RA\_L,R13\_L)}, \tty{(SP\_L,RA\_L)}, and \tty{(SP\_H,SP\_L)}.}
\item{Index mode addressing (\tty{[Rn]abs20, [Rn]disp(rp)}) is not allowed.}
\item{Some instructions allow shorter forms for the addressing mode \tty{disp(reg)}
      if the displacement is either zero or smaller than 0x4000.}
\end{itemize}
The runtime setting of this flag may be announced via a
\begin{verbatim}
   assume sr:n
\end{verbatim}
statenemt, with \tty{n} either being zero or one.  The default is zero, i.e. all
registers have their native width and index mode addressing is allowed.
...
\subsection{DN,DB,DW,DD,DQ,DT, and DO}
...
\item{\tty{DB}: 8-bit byte or ASCII string similar to \tty{DC.B}}
\item{\tty{DW}: 16-bit integer or half precision floating point}
\item{\tty{DD}: 32-bit integer or single precision floating point}
\item{\tty{DQ}: 64-bit integer or double precision floating point}
\item{\tty{DT}: packed BCD integer or extended precision floating point (80 bits)}
\item{\tty{DO}: quad precision floating point (128 bits)}
\end{itemize}
...
\item{In order to be compatible to the M80, \tty{DEFB/DEFW} may be used
      instead of \tty{DB/DW} in Z80-mode.}
\ttindex{BYTE}\ttindex{WORD}\ttindex{ADDR}\ttindex{ADDRW}
\item{\tty{BYTE/ADDR} resp. \tty{WORD/ADDRW} in COP4/8 mode are an
      alias for \tty{DB} resp. \tty{DW}, with the pairs differing in byte
...
\item{\tty{BYTE} resp. \tty{WORD} on PDP-11, VAX, WD16 work like \tty{DB}
      resp. \tty{DW}, however only accept integer arguments.}
\item{\tty{LWORD} resp. \tty{QUAD} on VAX work like \tty{DD} resp. \tty{DQ},
      however only accept integer arguments.}
\ttindex{HALF}
\item{\tty{BYTE} works on WE32xxx like \tty{DB}, \tty{HALF} like \tty{DW},
      and \tty{WORD} like \tty{DD}, however only stores integer values.}
\item{\tty{DOUBLE} works on WE32xxx like \tty{DQ}, however only stores
      floating point values.}
\ttindex{BYTE}\ttindex{STRING}
\ttindex{WORD}\ttindex{LONG}\ttindex{INT}
\ttindex{FLOAT}\ttindex{DOUBLE}
\item{The TMS340xx target provides:
      \begin{itemize}
      \item{\tty{BYTE} (like \tty{DB}, only integer arguments)}
      \item{\tty{STRING} (like \tty{DB}, only character string arguments)}
      \item{\tty{WORD} (like \tty{DW}, only integer arguments)}
      \item{\tty{LONG} or \tty{INT} (like \tty{DD}, only integer arguments)}
      \item{\tty{FLOAT} (like \tty{DD}, only floating point arguments)}
      \item{\tty{DOUBLE} (like \tty{DQ}, only floating point arguments)}
      \end{itemize}}
\ttindex{LWORD}\ttindex{QUAD}
\item{The Renesas RX target provides:
      \begin{itemize}
      \item{\tty{BYTE} (like \tty{DB})}
      \item{\tty{WORD} (like \tty{DW}, only integer arguments)}
      \item{\tty{LWORD} (like \tty{DD}, only integer arguments)}
      \item{\tty{FLOAT} (like \tty{DD}, only floating point arguments)}
      \item{\tty{DOUBLE} (like \tty{DQ}, only floating point arguments)}
      \end{itemize}}
\item{The CR16 target provides:
      \begin{itemize}
      \item{\tty{BYTE} or \tty{DC8} (like \tty{DB}, string or integer arguments)}
      \item{\tty{WORD} or \tty{DC16} (like \tty{DW}, only integer arguments)}
      \item{\tty{LONG} or \tty{DC32} (like \tty{DD}, only integer arguments)}
      \item{\tty{DC64} (like \tty{DQ}, only integer arguments)}
      \item{\tty{FLOAT} or \tty{DF32} (like \tty{DD}, only floating point arguments)}
      \item{\tty{DOUBLE} or \tty{DF64} (like \tty{DQ}, only floating point arguments)}
      \end{itemize}}
\end{itemize}
...
\tty{D1} works the same way as \ttindex{DB} and \ttindex{DN}.  The difference
however is that it defines or reserves only a single bit.  This instruction
therefore only makes sense if an address space is addressed bitwise.  Currently,
this is only the case for the 8051's {\tt BITDATA} segment if the {\tt BITSEGSIZE:1}
...
\tty{DC24} works the same way as \ttindex{DW} and \ttindex{DD}, however
stores (integer) values of 24 bits length.
...
\tty{FLT} works the same way as \ttindex{DD}, it however only stores floating
point numbers in IEEE single precision format.
...
with Motorola CPUs suggest \tty{DS} to be a built-in instruction.
...
\item{\tty{TFLOAT}: mantissa with 64 bits, exponent with 32 bits}
\end{itemize}
...
{\em valid for: TMS340xx}
 
\tty{FIELD} stores a bitfield of given length and value in memory. Its second
argument defines the field's length (1 to 32 bits), while the first argument
defines the (integer) value to be stored in the field.  The value's allowed
range depends on the field length and includes both unsigned and two's complement
signed values.  For instance, if the field length is 16 bits, allowed values
...
\subsection{ASCII, ASCIC, and ASCIZ}
...
\tty{EVEN} is a specialization of \tty{ALIGN}. The instruction increments the
program counter so it becomes a muliple of 16 bits.  This is for example the
proper alignment for processor instructions.
...
braces):
\begin{itemize}
\item{\tty{GLOBALSYMBOLS} resp. \tty{NOGLOBALSYMBOLS} control whether
      defined labels are local for every individual pass or not.}
\item{\tty{EXPAND} resp. \tty{NOEXPAND}}
\item{\tty{EXPIF} resp. \tty{NOEXPIF}}
\item{\tty{EXPMACRO} resp. \tty{NOEXPMACRO}}
\item{\tty{EXPREST} resp. \tty{NOEXPREST}}
\end{itemize}
...
defined:
\begin{itemize}
\item{\tty{IFDEF $<$expressionl$>$}: true if the expression does not contain
      any symbols that are undefined by the source assembled up to this
      point.  CAUTION! Querying forward declarations will return a 'false',
      also in the second and subsequent passes.}
\item{\tty{IFNDEF $<$expression$>$}: counterpart to \tty{IFDEF}.}
\item{\tty{IFSYMEXIST $<$symbol$>$} : true if the queried symbol exists
      in the symbol table.  CAUTION! Querying a forward declaration returns
      false in the first pass, but true in all subsequent passes.}
\item{\tty{IFNSYMEXIST $<$symbol$>$}: counterpart to \tty{IFSYMEXIST}.}
\item{\tty{IFUSED $<$symbol$>$}: true if if the given symbol has been
      referenced at least once up to now.  See section \ref{sect_symused}
      for exceptions from this.}
...
announced to be local, and \asname{} will only look in the symbol table's
part local to this section when this symbol is referenced.  In our
...
Opposed to \tty{PUBLIC} and \tty{GLOBAL}, \tty{FORWARD} may also be used
outside a section.  In this case, one of its properties is preserved: A
'placeholder entry' is added to the symbol table.  It does not contain an
actual value, and references to such a placeholder entry are treated like
accessing a (yet) undefined symbol.  The 'used' flag will however be set.
A \tty{FORWARD} declaration may therefore be useful if usage of it is
checked via \tty{SYMUSED}, and if there are references in the source code
...
which format it is written.  If \asname{} detects this instruction and no share
...
For historical reasons, the {\tt BITDATA} segment is by default {\em not}
actually organized in bits.  This means that a statement like this:
\begin{verbatim}
flag db ?
\end{verbatim}
will increase the program counter only by one, and not by eight.  There is
an implicit assumption that a byte in the {\tt BITDATA} segment is actually
only a bit, and if one is not only reserving memory, only the values zero and
one should be stored in these bytes.  This property is indeed not intuitive,
but is due to the fact that earlier versions of \asname{} did not support
address spaces with a word with other than eight, sixteen or thitry-two bits.
And when it became possible to handle other widths, changing the (default)
word size would have broken existing code.

So if the {\tt BITDATA} segment shall actually be organized in bits, this
has to be requested explicitly when selecting the target:
\begin{verbatim}
        cpu     8051:bitsegsize=1
\end{verbatim}
Furthermore, keep in mind that the output of P2BIN or P2HEX for this segment
will still use one byte per bit.
...
'derived' from Z80, like Z80UNDOC, Z180, Z280, RABBIT2000, and LR35902.
...
Similar to the Z280/Z380 and eZ80, it is possible to access the byte halves
...
The whole situation becomes complicated by by the usage of the instruction
prexixes \tty{DDIR W/LW/IB/IW}.  These allow to override the word size
and/or the length of immediate and absolute operands for the next instruction.
\asname{} memorizes the explicit usage of these prefixes and will take this
into account when assembling the following instruction.  The following
cases have to be differentiated:

If no \tty{DDIR} instruction preceded the current instruction, or a \tty{DDIR}
instruction that contained no explicit specification of operand length, the
operand length will be determined automatically.  For instance, a single
\begin{verbatim}
        LD      BC,12345678h
\end{verbatim}
will be augmented with a leading \tty{DDIR IW}.  However, in this case:
\begin{verbatim}
        DDIR    LW
        LD      BC,12345678h
\end{verbatim}
the existing \tty{DDIR} will be augmented with a \tty{IW}, and the code
generated is the same as for
\begin{verbatim}
        DDIR    LW,IW
        LD      BC,12345678h
\end{verbatim}
The code generated previously for \tty{DDIR LW} is withdrawn, as can be seen
from the \tty{R} marker in the listing.

Explicit \tty{DDIR} prefixes containing \tty{IN}, \tty{IB}, or\tty{IW}
may be used to enforce a certain operand length.  Arguments not fitting
into the given operand size result in an error message, while shorter
ones are zero-extended to achieve the requested length.  For instance,
\begin{verbatim}
        DDIR    IB
        LD      BC,12345678h
\end{verbatim}
will result in an error message, since the constant cannot be represented
in 24 bits.  Vice versa, in this example,
\begin{verbatim}
        DDIR    LW,IW
        LD      BC,345678h
\end{verbatim}
the constant will be encoded with one byte more than absolutely
necessary.
...
When National extended the CR16 architecture to 2 Mbytes address space,
compatibility to the CR16A predecessor was an issue.  The CR16B core therefore
has a 'compatibility bit' that provides full binary upward compatibility.  The
downside of this mode is that various branch and jump instructions still address
only the first 128 KBytes of address space.  The binary encoding of these
instructions changes in ''large model'', so the complete 2 MBytes address space
can be reached.  The \tty{CPU} selects the memory model code shall be created
for:
...
\begin{itemize}
\item{If the argument is larger than 15, it must be an address, otherwise:}
\item{If the argument is not a multiple of four, it must be a vector, otherwise:}
\item{If the argument is zero, no distinction is possible, otherwise:}
\item{If the argument is a symbol from CODE space (e.g. a label), it must
      be an address, otherwise:}
\item{The argument is a vector.}
\end{itemize}
...
are addressed with addresses 0,1,2,...65535.  Granularities
smaller than 8 bits are encoded asvalues of \$f9 to \$ff.  In case
the smalles addressablelement is one bit, the granularity is \$ff,
while it is \$fc for a nibble.
...
constructed by appending CMD to the program's name, i.e. \tty{PBINDCMD} for
PBIND), to negate options, and to use all upper- resp. lower-case
...
PBIND is a program that allows to concatenate the records of several
...
PBIND can also be used to split a code file into several files.

The general syntax of PBIND is
\begin{verbatim}
   PBIND [options] <source file(s)> [target file]
\end{verbatim}
Just like \asname{}, PBIND regards all command line arguments that do not
start with a \tty{+, -} or \tty{/} as file specifications.  If no
\tty{-o} was used, the last of them is assumed to be the target file.
All other file specifications name sources, which may contain wildcards.

Currently, PBIND defines only two command line options:
\begin{itemize}
\item{\tty{o $<$name$>$}: specifies the output/target file.}
\item{\tty{f $<$header[,header]$>$}: sets a list of record headers that should
      ...
      with commas.}
\end{itemize}
For example, to filter all MCS-51 code out of a code file, use PBIND
in the following way:
\begin{verbatim}
...
P2HEX is an extension of PBIND.  It has all command line options of PBIND and
uses the same conventions for source and destination file names.  Different to PBIND, the
target file is written as a Hex file, i.e. as a sequence of lines
which represent the code as ASCII hex numbers.

P2HEX knows ten different target formats, which can be selected via the
...
In contrast to PBIND, P2HEX will not produce an empty target file if
...
\errentry{ 490}{interpreting too large integer constant as float}
               ...
               {Though the argument is a syntactically correct integer constant,
                ...
                number.}
               {the argument in question}
\errentry{ 500}{code generation outside of CODE segment}
               {warning}
               {machine code can only be generated inside the CODE segment. All
                other address spaces do not have the necessary word width.}
               {none}
\errentry{ 510}{instruction will overwrite SPt}
               {warning}
               {Aside from the explicitly named register, this instruction
                uses the subsequent register for the double-length result.
                However, the subsequent register would be the stack pointer.}
...
\errentry{1440}{invalid control register}
               ...
               {none}
\errentry{1441}{unknown vector}
               {error}
               {A vector of this name does not exist.}
               {the vector argument in question}
\errentry{1442}{register is not accessible in current execution modee}
               {error}
               {This register may only be accessed in kernel mode.}
               {The register in question}
\errentry{1443}{register is read-only in current execution mode}
               {error}
...
\errentry{1480}{invalid IF-structure}
               ...
               {The command structure in a \tty{IF}- or \tty{SWITCH}- sequence is
                wrong.}
               {none}
\errentry{1482}{FORWARD statement must reference to current section}
               {error}
               {\tty{FORWARD} directives must not reference symbols in
                another (higher) section.}
               {The symbol name in question}
...
\errentry{1520}{only ON/OFF allowed}
               ...
               {This pseudo command accepts as argument either \tty{ON} or
                \tty{OFF}}
               {none}
\errentry{1521}{invalid LISTING value}
               {Fehler}
               {Only \tty{ON}, \tty{OFF}, \tty{NOSKIPPED}, or
                \tty{PURECODE} is allowed as argument for \tty{LISTING}.}
               {the argument in question}
...
\errentry{1804}{WHILE without ENDM}
               ...
               {An incomplete WHILE block was found. Probably an
                \tty{ENDM} statement is missing.}
               {none}
\errentry{1805}{EXITM used outside of macro}
               {error}
               ...
               {The \tty{EXITM} statement in question}
\errentry{1806}{ENDM used outside of macro}
               {error}
               {\tty{ENDM} finalizes either a macro's definition or a
                \tty{REPT/IRP/WHILE} block.  Using it outside of either
                makes no sense.}
               {The \tty{ENDM} statement in question}
...
\errentry{2270}{page not accessible}
               ...
               {The target address is located in a memory page that is currently
                inaccessible.}
               {none}
\errentry{2271}{current program counter in inaccessible page}
               {error}
               {The instruction currently being assembled is located on a physical
                address that is not mapped into the CPU's logical address space.
                This is an issue if PC-relative addressin is used, because the
                logical address is needed to compute the distance.}
               {none}
...
\newcommand{\pcsymname}[0]{Program Counter Symbol}
...
Christian Stelter, Patrik Str\"omdahl, Tadashi G. Takaoka,
Oliver Thamm, Thorsten Thiele, Leszek Ulman, Rob Warmelink,
